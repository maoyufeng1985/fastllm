...
**8K 并发聚合的"持平"是假象，U0 已证实（2026-09-15）**：C=1/C=2/C=4 的
common window 分别 87.0 / 87.17 / 86.96，看着完全持平；但这张表是在
`--tokens 16384` 下测的，窗口里只有单请求，而"C=2 单请求 43.6"是把 87.17
对半折算的假设值。**U0 三次同 sha 的复核给出真双请求窗口**（`--tokens 32768`、
out=1024）：每请求 **81.6 tok/s**、聚合 **163 tok/s**、并发步长 **≈12.3 ms
= 1.07× C=1**，即**权重近似全额摊销**。按 2.00× 不摊销算每步该是 ~23 ms、
聚合 ~43 tok/s，实测差 3.7 倍。

**所以「8K 撞权重带宽墙、并发无可捡空间」作废**，`sm70_1cat_port_plan.md`
§3 的 8K 持平表与 `sm70_concurrency_port_plan.md` 的 53.58 解释按此回填。
步长口径注意：`common window` 的聚合 tok/s 是两请求之和，**步长要用每请求
token 数去除窗口跨度**（12.53 s / 1021 ≈ 12.3 ms），用聚合数除会得到 6.1 ms 的
"每聚合 token 时间"，不是步长。裁决行见 `sm70_c2_ttft_overlap_plan.md` §0.5。
