...
两张卡的内存账（用实测口径：总 16.93 GB，`reserved` 0.51、`runtimeHeadroom`
0.537、`servingReserve` 0.134 GB）：

| | TP4 | TP5 |
|---|---:|---:|
| 每卡权重（20.56 GB 分片） | 5.14 GB | 4.11 GB |
| 可给池子 + 激活 | 10.61 GB | 11.64 GB |

4 × 200K 需要 6252 页 = **6.56 GB** KV（FP8），加 80% 上限得池子 **7815 页 =
8.21 GB**。而实测池子上限受物理显存约束：请求 15625 页（16.4 GB）直接
`cuda malloc failed`；能跑成的 3200 页池峰值已到 **15711 MiB，余量 673 MiB**。

**TP5 只在权重上省 1.03 GB/卡，补不上 8.21 − 3.36 = 4.85 GB 的池子缺口。**
而且多出的第 5 张卡在 TP4 布局下是闲置的。

**能真正解决 4 × 200K 的只有 context parallel**（把每条序列的 KV 切开）：
CP2 让每卡只存 100K，4 × 200K 的 KV 减半到 3.28 GB，`TP4 + CP2` 需要 8 张卡。
代价是每个全注意力层要跨卡交换 K/V，而本机 **NVLink 全 inactive**、走 PCIe——
200K 下每层每卡要换 328 MB，16 层约 5.2 GB/step，按 PCIe 量级是**几十到几百
毫秒每步**，而当前整个 decode 步才 12–17 ms。**所以即使加卡，PCIe 也会先成为墙。**
