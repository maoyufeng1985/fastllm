...
### Step 2. prefill chunk 尺寸 A/B（2048 / 4096 / 8192）

更大的 chunk = 更少的调度轮、更大的 GEMM、更少的固定开销，是唯一"不动内核就能
减 prefill 总时长"的现成杠杆。

- 验证门：总时长最低的档位；且内存放得下（当前 4×200K 校准后每卡只剩 326 MB，
  warmup 缓冲随 chunk 放大，必须逐档盯 gpuFree）。
- 预期：8192 档如果内存扛得住，调度轮从 ~98 降到 ~25，总时长有望明显下降。
