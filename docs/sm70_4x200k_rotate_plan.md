...
**Step 4 结论（第五轮，最终）：** GEMM 内核本身没有 3 倍空间（45.6–61 TFLOPS
对实测 97.3 墙）；NCCL 也不是"没藏好"那么简单——引擎运行在**队列深度 1**
（四卡各自 107 万内核同时最多 1 个在飞、重叠 0.00 s；主机每线程 82% 时间被
同步挡住），所以通信与计算想重叠，必须先改主机提交模型，而不是换个流。
分块重叠在理想提交模型下的上限是 **14–15%**（nsplit=8），落到 104.2 s 上约
5–6 s。4b 的"RS+AG 流量减半"经实测为错（环上流量相同，还慢 6%）。
此前三版估算（"没有白捡的内核优化"、"GEMM 只跑到墙的 1/4"、以及 −35%/−23%/
−6%）全部作废。
...
| **Step 4a′ all-reduce 藏进计算**（第五轮，引擎执行模型下定性） | **最多 14–15%（约 5–6 s，104.2 → ~99 s）**，且必须先改主机提交模型。分块重叠（nsplit=8，合法依赖）实测 14–15%，但那是"主机提前提交全部工作"的理想模型；引擎实际是**队列深度 1**——device 0 上 1077724 个内核同时最多 1 个在飞、重叠 0.00 s，主机 82% 时间被同步挡住（97 s / 118 s），所以只换 side stream **拿不到任何收益** | 需同时做两件事：① AR 挪 side stream；② 提高队列深度（按层批量提交、去掉逐算子主机等待）。②是主机执行模型重构，改动面远大于"换个流" | 未做；−35% / −23% / −6% 三版估算全部作废，实际上限 14–15% 且需重构 |
