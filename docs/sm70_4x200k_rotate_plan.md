...
all-reduce 与计算重叠四条路全部实测为 **0 或负收益**（详见 §6 收益表）。
**第九/十轮更正了原文写的根因**：原文说"设备侧单流串行（`max_concurrent=1`，
从不并发），所以重叠不可能发生"——**这个前提是错的**。实测（§9.1）显示，把
all-reduce 放到侧流后，设备上**真的出现了两个内核同时飞，占窗口 23.3%**；
同场墙钟反而慢 7.4%。而且**主机排空也已被排除**：R1 把每 rank 的
`cudaStreamSynchronize` 排空从 **29.06 s 压到 0.39 s**（引擎普查，§10.6），
墙钟却从 37.54 s 涨到 40.05 s。所以正确说法是"**并发发生了、主机阻塞也消掉了，
但墙钟没动**"，两个候选根因都被实测排除。剩下的方向见 §10.6。
**要动总时长，只剩"减少每条请求的上下文"这一条**（200K→180K 省 10%、→100K 省
50%，一行配置，无需改代码）。
...
1. **NCCL 不是肉。** ~~41.9 MB 消息、5.38 ms 一次 → busbw ≈ 11.7 GB/s，已是
   PCIe3 x16 实用峰值（~12–13 GB/s）的九成~~ **【此数字已撤，见 §10.2】**：
   那个 5.38 ms 是**所有尺寸混在一起的平均**，而 41.9 MB 那一桶的真实均值是
   **10921 µs**（差 2.03 倍），按尺寸读出的 busbw 是 **5.76 GB/s**。
   这 33% 是 TP4 over PCIe（无 NVLink）的结构成本。
...
| R1 `SPLITS=2`（§10.4，另一场的同场实测） | 38.9770 s | 2060.44 tok/s | **+3.92%** |
| R1 `SPLITS=4`（§10.4） | 39.9737 s | 2011.65 tok/s | **+6.58%** |
| R1 `SPLITS=8`（§10.4） | **OOM，无数据** | — | — |

**分块切得越细亏得越多**（同场实测 `SPLITS=2/4` 分别 +3.92% / +6.58%，§10.4），
与 §10.3 的普查读数一致：切分把集合通信次数从 10240 抬到 81920。
...
## 10. 用引擎自带仪器复核：AR 真实尺寸/耗时，以及一次旧数字撤错（2026-09-17）

上一节靠 nsys 内核推断 AR 的尺寸和耗时，那是**推**。这一节改用引擎自带的
普查（`FASTLLM_NCCL_AR_STATS=1`，计数器在 `FastllmNcclAllReduceImpl` 内部记录
字节数与纳秒数，分子分母同源），拿到的是**直接读数**。80K prompt、TP4、device 0。

### 10.1 对照（无 R1）

`Total 37.5393 s`，sha `1d23759c`。

```
  bytes           calls   total ms      mean us busbw GB/s oneway GB/s
  10240            6144     1475.4       240.13       0.06       0.04
  20480             512       38.1        74.43       0.41       0.28
  40960             512       35.1        68.63       0.90       0.60
  81920             512       39.8        77.69       1.58       1.05
  163840            512       56.2       109.72       2.24       1.49
  22282240          512     3049.1      5955.32       5.61       3.74
  41943040        10240   111834.1     10921.29       5.76       3.84
```

**prefill 的 AR 就是 41,943,040 B（41.9 MB）× 10240 次，单次 10921 µs，
busbw 5.76 GB/s**（`oneway 3.84`）。注：表头的 counts/times 是**四卡求和**，
每卡除以 4（10240/4 = 2560）。

耗时去向（同一份读数）：

```
  pre-rendezvous(Before)                  8.4 ms    0.0%
  submit+After-rendezvous               365.3 ms    0.3%
  post-sync-setup                         1.7 ms    0.0%
  drain(cudaStreamSynchronize)       116233.1 ms   99.7%   <- 几乎全是排空
```

### 10.2 撤错：文档里那个"busbw ≈ 11.7 GB/s"站不住

§4 的归因表写过"NCCL AllReduce：每卡 6656 次、平均 5.38 ms、消息 41.9 MB →
busbw ≈ 11.7 GB/s，已是 PCIe3 x16 实用峰值的九成"。

**撤错内容**：那个 5.38 ms 是**所有尺寸混在一起的平均**，而 41.9 MB 那一桶的
真实均值是 **10921 µs**，差 **2.03 倍**。用 41.9 MB / 5.38 ms 算出的 11.7 GB/s
因此是错的（也正是本项目规约里"对多峰分布直接求均值"那一禁条的现实案例）。
**正确的按尺寸读数是 busbw 5.76 GB/s**（引擎普查，分子分母同源）。

撤错后"NCCL 不是肉"这个**结论方向不变**：按尺寸看，41.9 MB 的 busbw 只有
5.76 GB/s，离 PCIe3 x16 实用上限（~12–13 GB/s）还差一半，且这份耗时里
**99.7% 是主机 `cudaStreamSynchronize` 排空**（每 rank 29.06 s / 墙钟 37.54 s）。
但**"排空是瓶颈"这个说法随后被 §10.6 的实测否定**：R1 把排空压到 0.39 s/rank，
墙钟没降反升。所以这里只保留"11.7 GB/s 这个数错了"，不再由它推出瓶颈在哪。

### 10.3 R1（分块）下的 AR：尺寸被精确切分

`FASTLLM_TP_AR_SIDE_STREAM=1 + _PIPELINE=1 + _SPLITS=8`，`Total 40.0493 s`，
sha 同。41.9 MB 那一桶从 drained 表**消失**，async 表出现：

```
  bytes           calls   total ms      mean us busbw GB/s oneway GB/s
  2785280          4096       51.6        12.61     331.33     220.89
  5242880         81920     1011.7        12.35     636.81     424.54
```

读数：**41943040 ÷ 8 = 5242880 精确成立**，**10240 × 8 = 81920 精确成立**。
即 SPLITS=8 把每条消息切成 8 片、每片发一次集合通信。
交叉验证：81920 / 4 卡 = **20480** 个内核/卡，与 §9.1 在 stream 423 上数到的
20480 个内核**吻合**。

（async 表的 `mean us` 是**提交**耗时、不是完成耗时，所以那两列的 GB/s 无意义，
引擎自己的表头也这么写。这里只用它来确认"尺寸与次数"，不用它谈速率。）

### 10.4 splits 扫描（同场串行实测）

80K prompt、TP4、chunk 4096、FP8 KV、`FASTLLM_QWEN35_SM70_CUDA_GRAPH=0`，
同一次会话内串行跑（`/tmp/sw2_*.out`）：

| 配置 | Total | Prefill | 相对对照 | sha256 |
|---|---:|---:|---:|---|
| 对照（无 R1） | **37.5068 s** | 2141.60 tok/s | — | `1d23759c` |
| `SPLITS=2` | 38.9770 s | 2060.44 tok/s | **+3.92%** | `1d23759c` |
| `SPLITS=4` | 39.9737 s | 2011.65 tok/s | **+6.58%** | `1d23759c` |
| `SPLITS=8` | **OOM，无数据** | — | — | — |

对照片本会话跑过 4 次：37.4720 / 37.5068 / 37.5393 / 37.6385 s，
极差 0.167 s，所以 +3.92% / +6.58% 都远在噪声之外。

`SPLITS=8` 那一臂**未完成**：CUDA OOM
（`cudaErrorMemoryAllocation`，`requestBytes 51216384`，`float8 [1563,128,1,256]`），
不是死锁、不是硬件故障；设备当时仍是 4 张。**该臂无数据。**

**结论（实测）**：分块 R1 在每一个测过的 splits 值上都亏，且切得越细亏得越多。
与 §10.3 的读数一致——切分把集合通信次数从 10240 抬到 81920。

### 10.5 安全记录：两次 R2 崩溃（待人工判断）

本会话内四张卡**掉卡两次**，两次都发生在**同一个配置**上：

| 次 | 时间 | 正在跑的配置 | 结果 |
|---|---|---|---|
| 1 | 09:25 | `FASTLLM_TP_AR_SIDE_STREAM=1`（R2，无分块） | 四卡 AER 不可纠正错误 + Xid 79/154 |
| 2 | 09:52 | `FASTLLM_TP_AR_SIDE_STREAM=1`（R2，无分块） | 同上 |

而未崩溃的是带分块的配置（`+ _PIPELINE=1`），本会话 2/2 正常完成。

**这是相关性，不是已证因果。** 崩溃签名是 **PCIe 链路级**（`pcieport AER`、
`nvidia: AER: can't recover`、`Xid 79 GPU has fallen off the bus`），而本机
**同时有另一个代理在做 FlashInfer `pcie_ipc`**（那是直接走 PCIe 点对点写入的
负载），时间上有重叠；从本会话内部无法归因。**故不再重跑 R2**，只跑从未崩过的
分块配置；R2 的相关性留作待查。

### 10.6 关键实测：R1 把主机排空几乎消掉，墙钟却没动

引擎普查的 drained 表头写明"counts and times are **SUMMED OVER ALL RANK
THREADS**，divide by the rank count for a per-rank figure"，按此换算：

| 配置 | drained 总计（四卡和） | 每 rank | 其中 `drain(cudaStreamSynchronize)` 每 rank | 墙钟 |
|---|---:|---:|---:|---:|
| 对照（无 R1） | 116.61 s | **29.15 s** | **29.06 s** | **37.5393 s** |
| R1（`_PIPELINE=1 _SPLITS=8`） | 2.82 s | 0.70 s | **0.39 s** | **40.0493 s** |

**这是一条干净的否证。** 对照里每 rank 有 **29.06 s** 花在集合通信后的
`cudaStreamSynchronize` 排空上（占墙钟 37.54 s 的 77%）。R1 把侧流接上之后，
这个排空**降到 0.39 s**——几乎完全消失。**如果主机排空是关键路径，墙钟应该
大幅下降；实测是反而从 37.5393 s 涨到 40.0493 s（+6.7%）。**

所以：
- **"主机阻塞是关键路径"这个假设被实测排除**（此前 §8.1 与 §8.3 都把它当作
  并发上不去的原因）。
- R1 消掉的排空是**被设备侧工作盖住的**，不在关键路径上。
- 它涨上去的那 6.7%，与 §10.3 的读数同源：切 8 片把集合通信次数从 10240
  抬到 81920，碎包开销超过了省下的排空。

**两个候选根因现在都被实测排除**：(a) "并发不可能发生"（§9.1 否掉）、
(b) "主机排空是瓶颈"（本节否掉）。**下一步该往哪走，本会话没有数据**——
不再据此推断。

## 11. 结论：R1 / R2 的收益（2026-09-17）

口径统一：80K prompt、`--output_tokens 8`、4×V100、TP4、chunk 4096、FP8 KV、
`FASTLLM_QWEN35_SM70_CUDA_GRAPH=0`、同机同会话串行。sha256 全部为 `1d23759c`。

### 11.1 收益表（数字栏全部为实测值）

| 路线 | 开关 | Total | 相对对照（**正 = 更慢**） | 状态 |
|---|---|---:|---:|---|
| **对照** | 无 | **37.5068 s** | — | 基线 |
| **R2**：AR 放独立流、不分块 | `FASTLLM_TP_AR_SIDE_STREAM=1` | **39.9610 s** | **+7.04%** | 已实现、已实测、**默认关** |
| R1：R2 + 分块流水 `SPLITS=2` | `_PIPELINE=1 _SPLITS=2` | 38.9770 s | **+3.92%** | 已实现、已实测、默认关 |
| R1：`SPLITS=4` | `_PIPELINE=1 _SPLITS=4` | 39.9737 s | **+6.58%** | 同上 |
| R1：`SPLITS=8` | `_PIPELINE=1 _SPLITS=8` | 40.4062 s | **+7.35%** | 同上 |
| R1：`SPLITS=8`（另一场复核） | 同上 | 40.0493 s | +6.69% | 同上 |
| `SPLITS=8`（再一场） | 同上 | **OOM，无数据** | — | — |

对照片四次：37.4720 / 37.5068 / 37.5393 / 37.6385 s，**极差 0.167 s**。
所以 R2 的 +7.04%、R1 各档的 +3.92%～+7.35% 都**远在噪声之外**。

（符号约定与 §8.2 一致：正数 = 比对照慢。本表初版误写成负号且写作
−7.7%/−6.8%，实为 +7.35%/+6.69%，已更正。）

**一句话**：**R2 和 R1 都是负收益，R2 最亏（+7.04%）；R1 的分块能把亏损收窄
（分块确实回收了一部分），但没有一档能翻正。** 这就是这两条路的收益结论。

（R2 的 39.9610 s 引自 §8.2；本会话另跑的对照 37.4149 s 与 §8.2 的 37.3322 s
同量级，可见两场的基线可比。）

### 11.2 那个"`drain` 占 99.7%"该怎么读——**它不是发现，是仪器的定义**

引擎普查（`FASTLLM_NCCL_AR_STATS=1`）里有一行 `drain(cudaStreamSynchronize)
116233.1 ms / 99.7%`。这个数**不能当成"瓶颈是排空"的依据**，原因：

普查表头自己写着 `drained = submission plus the per-collective
cudaStreamSynchronize`。**这张表的 TOTAL 按定义就等于（提交 + 排空）**，所以
两者相加当然接近全部——把一个加法恒等式读成"发现瓶颈"，是我的错。

它唯一有用的信息是**另一半**：`submit+After-rendezvous` 只占 **0.3%**，
说明**主机侧发射开销可以忽略**（不是瓶颈）。

而**"排空是不是瓶颈"这个问题本身，已经由 R1 的实测回答**（§10.6）：
R1 把每 rank 排空从 **29.06 s 压到 0.39 s**（几乎消掉），墙钟却从 37.54 s
**涨到** 40.05 s。**排空不在关键路径上**——它本来就被设备侧工作盖住了。

所以正确读法是三句：

1. `99.7%` = 恒等式，不是结论；
2. `submit 0.3%` = 主机发射开销可忽略（有用）；
3. 排空**不是**关键路径，这由 §10.6 的 A/B 直接证明，不由 99.7% 推出。

### 11.3 本会话两条被实测排除的假设

| 假设 | 来源 | 实测结果 | 判据 |
|---|---|---|---|
| "并发不可能发生、队列深度 1" | §8.1/§8.3 | **否**：R1 下 `t≥2 = 10.5546 s`（窗口 23.3%）、`max_concurrent=2` | §9.1 |
| "主机排空是关键路径" | §8.1/§8.3、以及我据 99.7% 的推断 | **否**：R1 把 29.06 s/rank 排空压到 0.39 s，墙钟反升 | §10.6 |

**下一步往哪走，本会话没有数据**，不再据此推断。

### 11.4 R1 / R2 的当前状态：**冻结，不再推进（2026-09-17）**

**决定：R1 与 R2 的代码原样留在树里，但不再做任何后续实现或测量。**
理由是实测已给出结论（§11.1：两条路全负收益），继续投入没有回报。

| 项 | 状态 |
|---|---|
| 实现 | **已在树里，原样保留**（不删、不改） |
| 默认行为 | **默认关**，不影响任何正常跑法 |
| 收益 | 已测完：R2 **+7.04%**；R1 `SPLITS=2/4/8` 分别 **+3.92% / +6.58% / +7.35%**（正 = 更慢） |
| 后续 | **无。不再实现、不再调参、不再复跑** |

**开关清单（全部默认关，不设环境变量即走原路径）**

| 开关 | 作用 |
|---|---|
| `FASTLLM_TP_AR_SIDE_STREAM=1` | R2：把 TP all-reduce 放到每 (卡,线程) 一条侧流上 |
| `FASTLLM_TP_AR_SIDE_STREAM_PIPELINE=1` | R1：在 R2 基础上把行并行输出按行分块 |
| `FASTLLM_TP_AR_SIDE_STREAM_SPLITS=N` | 分几块（默认 4） |
| `FASTLLM_TP_AR_SIDE_STREAM_HOST_SYNC=1` | 诊断用：把侧流路径省掉的排空加回来 |
| `FASTLLM_TP_AR_CUSTOM_AR_ON_SIDE_STREAM=1` | R5：自定义 AR 走侧流（**会崩，禁止开启**） |

**两次没跑出结果的尝试（如实记录，不当作数据）**

| 尝试 | 结果 | 现场 |
|---|---|---|
| `SPLITS=8` 一臂 | **CUDA OOM**（`cudaErrorMemoryAllocation`，`requestBytes 51216384`） | 设备仍 4 张，非硬件故障 |
| R2 复核（`SIDE_STREAM=1` 单独） | **挂住，无数据** | 跑满 400 s（正常 ~40 s），四卡 util 全 0% 但各占 5.5 GB；python 37 线程中主线程等 10 个卡在 `futex_do_wait`、3 个自旋、13 poll、11 nanosleep —— **主机侧锁/自旋死锁**，设备无新 Xid |

**因此 §11.1 表里 R2 的 39.9610 s 用的是 §8.2 已测过的值**（引自
`docs/sm70_4x200k_rotate_plan.md:438`），**不是**这次挂住的复核跑。这次没产出数。

**安全备注**：本会话四卡掉卡两次，两次都发生在 `FASTLLM_TP_AR_SIDE_STREAM=1`
（R2）上，且分别在 09:25 与 09:52；带分块的 R1 配置未发生过掉卡。这是**相关性、
非已证因果**（崩溃签名是 PCIe 链路级，本机同期另有代理在跑 FlashInfer `pcie_ipc`），
但因已决定冻结 R1/R2，**不再复现、不再追查**。R2 既有疑似崩溃史、又有死锁史，
**这也是"不再开启"的一个操作理由**。
