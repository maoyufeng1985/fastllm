...
- no-end 的墙钟收益还没在引擎 decode 上量过。12.94 vs 16.05 是探针数字。引擎里
  ...
  → **已量（2026-09-15，当前二进制，同形状 AR off/auto 对照）**：8K C=1
  79.35 → 87.00（**+9.6%**）、80K C=1 67.98 → 73.51（**+8.1%**）、
  8K C=2 149.86 → 161.72（**+7.9%**）。三对的 greedy sha256 都逐字相同
  （`02c702ca` / `9bb0aa71` / `5bfaac89`），所以这 +8% 到 +10% 是纯墙钟收益、
  不换数值。probe 的 19%（12.94 对 16.05）在引擎里兑现成了约 +8%。
